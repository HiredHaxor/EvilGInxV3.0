...
func printUpdateName() {
	...
	txt := nameClr.Sprintf("               - --  Advanced Edition  -- -")
...
func printOneliner1() {
	...
	txt := textClr.Sprintf("      by EvilDeaD (") + handleClr.Sprintf("@EvilDeadOfficial") + textClr.Sprintf(")") + spc + textClr.Sprintf("version ") + versionClr.Sprintf("%s", VERSION)
	fmt.Fprintf(color.Output, "%s", txt)
}
